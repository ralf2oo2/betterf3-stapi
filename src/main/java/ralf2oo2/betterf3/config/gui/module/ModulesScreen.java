...
public class ModulesScreen extends Screen implements IOnCloseHandler {
    ...
    public ModulePosition side;

    public ModulesScreen(Screen parent, ModulePosition side){
        this.parent = parent;
        this.side = side;
    }
    ...
    public void init() {
        ...
        if(this.side == ModulePosition.LEFT){
            this.modulesListWidget.setModules(Betterf3.f3State.leftModules);
        } else if (this.side == ModulePosition.RIGHT){
            this.modulesListWidget.setModules(Betterf3.f3State.rightModules);
        }
    }
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        this.setActiveState(this.modulesListWidget.selectedModuleIndex != -1);
    ...
    public void onClose() {
        if (this.side == ModulePosition.LEFT) {
            Betterf3.f3State.leftModules.clear();
            for (ModuleListWidget.ModuleEntry entry : this.modulesListWidget.moduleEntries) {
                Betterf3.f3State.leftModules.add(entry.module);
            }
        } else if (this.side == ModulePosition.RIGHT) {
            Betterf3.f3State.rightModules.clear();
            for (ModuleListWidget.ModuleEntry entry : this.modulesListWidget.moduleEntries) {
                Betterf3.f3State.rightModules.add(entry.module);
            }
        }
