...
public class LocationModule extends BaseModule{
    ...
    public void update(Minecraft minecraft) {
        String dimensionId;
        String biome = minecraft.world.method_1781().method_1787((int)Math.floor(minecraft.player.x), (int)Math.floor(minecraft.player.y)).name;
        int lookDir = (MathHelper.floor((double)(minecraft.player.yaw * 4.0F / 360.0F) + 0.5) & 3);
        //the output of this seems incorrect
        int topY = minecraft.world.getTopY((int)Math.floor(minecraft.player.x), (int)Math.floor(minecraft.player.z));
        ...
        lines.get(1).setValue(TranslationStorage.getInstance().get("text.betterf3.line." + directions[lookDir]) + " (" + TranslationStorage.getInstance().get("text.betterf3.line." + coord_directions[lookDir]) + ")");
        ...
        Chunk chunk = minecraft.world.method_199((int)Math.floor(minecraft.player.x), (int)Math.floor(minecraft.player.z));
        int blockLight = chunk.getLight(LightType.BLOCK, ((int)Math.floor(minecraft.player.x)) & 15, (int)Math.floor(minecraft.player.y), ((int)Math.floor(minecraft.player.z)) & 15);
        int skylight = chunk.getLight(LightType.SKY, ((int)Math.floor(minecraft.player.x)) & 15, (int)Math.floor(minecraft.player.y), ((int)Math.floor(minecraft.player.z)) & 15);
        //String.valueOf(minecraft.player.world.method_255((int)Math.floor(minecraft.player.x), (int)Math.floor(minecraft.player.y), (int)Math.floor(minecraft.player.z)))
        //minecraft.world.field_202
        lines.get(3).setValue(String.format(TranslationStorage.getInstance().get("format.betterf3.chunklight", Math.max(skylight, blockLight), skylight, blockLight)));
